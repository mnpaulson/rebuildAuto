...
namespace Assets.Editor
{
    ...
    {
        private static readonly string[] DevelopmentConfigFiles =
        {
        ...
        private sealed class DevelopmentImportSelection
        {
            public readonly HashSet<string> Maps = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            ...
            public readonly HashSet<string> EffectNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            public readonly HashSet<string> EffectSounds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            public readonly List<PlayerClassData> Jobs = new List<PlayerClassData>();
        }

        public static List<string> ValidateDevelopmentProfile(RagnarokDevelopmentCopyProfile profile)
        {
            ...
            foreach (var path in DevelopmentConfigFiles.Where(path => !File.Exists(path)))
                errors.Add($"Required generated configuration is missing: {path}");

            if (profile.all)
            {
                if (!File.Exists(Path.Combine(dataDir, "prontera.gat")))
                    Debug.LogWarning("[Development Copy] The Ragnarok data directory is missing prontera.gat.");
                if (!File.Exists(Path.Combine(dataDir, "texture/워터/water000.jpg")))
                    Debug.LogWarning("[Development Copy] The Ragnarok data directory is missing texture/워터/water000.jpg.");
                return errors;
            }

            if (errors.Count == 0)
                BuildDevelopmentImportSelection(profile, dataDir, errors);
            return errors;
        }

        public static void CopyDevelopmentProfile(RagnarokDevelopmentCopyProfile profile)
        {
            var errors = ValidateDevelopmentProfile(profile);
            if (errors.Count > 0)
            {
                EditorUtility.DisplayDialog("Development Copy Profile", string.Join("\n", errors), "OK");
                return;
            }

            var dataDir = RagnarokDirectory.GetRagnarokDataDirectorySafe;
            var prompt = profile.all
                ? $"Run the exhaustive '{profile.name}' client-data copy? This can take more than an hour."
                : $"Import the '{profile.name}' development profile? Existing imported assets will be kept.";
            if (!EditorUtility.DisplayDialog("Development Copy Profile", prompt, "Import", "Cancel"))
                return;
            ...
            try
            {
                if (profile.all)
                {
                    CopyFullDevelopmentProfileData(dataDir);
                    EditorUtility.DisplayDialog("Development Copy Profile", $"Profile '{profile.name}' is complete.", "OK");
                    return;
                }

                var selectionErrors = new List<string>();
                var selection = BuildDevelopmentImportSelection(profile, dataDir, selectionErrors);
                var copied = CopyDevelopmentRawFiles(dataDir, selection);
                ...
                Debug.Log(
                    $"[Development Copy: {profile.name}] Complete. Copied {copied} raw file(s), " +
            ...
            catch (Exception exception)
            {
                ...
                EditorUtility.DisplayDialog(
                    "Development Copy Profile Failed",
        ...
        private static DevelopmentImportSelection BuildDevelopmentImportSelection(
            RagnarokDevelopmentCopyProfile profile,
            string dataDir,
            List<string> errors)
        {
            var selection = new DevelopmentImportSelection();
            ...
            try
            {
                var maps = JsonUtility.FromJson<Wrapper<ClientMapEntry>>(File.ReadAllText(DevelopmentConfigFiles[0])).Items;
                ...
                foreach (var map in selection.Maps)
                    foreach (var extension in new[] { ".gnd", ".gat", ".rsw" })
                        if (!File.Exists(Path.Combine(dataDir, map + extension)))
                            Debug.LogWarning($"[Development Copy] Map source file is missing: {map}{extension}");

                var monsterDatabase = JsonUtility.FromJson<MonsterDbFile>(File.ReadAllText(DevelopmentConfigFiles[2]));
                var monsterClasses = JsonUtility.FromJson<Wrapper<MonsterClassData>>(File.ReadAllText(DevelopmentConfigFiles[1]))
                ...
                var npcDatabase = JsonUtility.FromJson<NpcDbFile>(File.ReadAllText(DevelopmentConfigFiles[3]));
                ...
                var playerClasses = JsonUtility.FromJson<Wrapper<PlayerClassData>>(File.ReadAllText(DevelopmentConfigFiles[5])).Items;
                ...
                var skillTrees = JsonUtility.FromJson<Wrapper<ClientSkillTree>>(File.ReadAllText(DevelopmentConfigFiles[7])).Items;
                ...
                var effects = JsonUtility.FromJson<EffectTypeList>(File.ReadAllText(DevelopmentConfigFiles[8])).Effects;
                ...
                var items = JsonUtility.FromJson<Wrapper<ItemData>>(File.ReadAllText(DevelopmentConfigFiles[4])).Items;
                ...
                var skills = JsonUtility.FromJson<Wrapper<SkillData>>(File.ReadAllText(DevelopmentConfigFiles[6])).Items;
                ...
                foreach (var sprite in selection.MonsterSprites)
                    WarnForMissingSpritePair(Path.Combine(dataDir, "sprite/몬스터"), Path.GetFileNameWithoutExtension(sprite));
                foreach (var sprite in selection.NpcSprites)
                    WarnForMissingSpritePair(Path.Combine(dataDir, "sprite/npc"), sprite);
                foreach (var job in selection.Jobs)
                {
                ...
                var heads = JsonUtility.FromJson<Wrapper<PlayerHeadData>>(File.ReadAllText(DevelopmentConfigFiles[9])).Items;
                if (heads.Length == 0)
                    Debug.LogWarning("[Development Copy] No default hairstyle is defined in headdata.json.");
                else
                {
                    foreach (var head in heads[0].MaleIds)
                        WarnForMissingSpritePair(Path.Combine(dataDir, "sprite/인간족/머리통/남"), head);
                    foreach (var head in heads[0].FemaleIds)
                        WarnForMissingSpritePair(Path.Combine(dataDir, "sprite/인간족/머리통/여"), head);
                }
                ...
                foreach (var file in RagnarokClientDataImportDefinitions.MiscellaneousFiles)
                    if (!File.Exists(Path.Combine(dataDir, file.SourceRelativePath)))
                        Debug.LogWarning($"[Development Copy] Baseline file is missing: {file.SourceRelativePath}");
            }
            catch (Exception exception)
            {
        ...
        private static void WarnForMissingSpritePair(string sourceDirectory, string spriteName)
        {
            ...
            {
                Debug.LogWarning($"[Development Copy] Sprite source directory is missing: {sourceDirectory}");
                return;
            }
            ...
            foreach (var extension in new[] { ".spr", ".act" })
                if (!Directory.EnumerateFiles(sourceDirectory, "*", SearchOption.TopDirectoryOnly).Any(path =>
                    ...
                    Debug.LogWarning($"[Development Copy] Sprite file is missing: {spriteName}{extension}");
        }

        private static int CopyDevelopmentRawFiles(string dataDir, DevelopmentImportSelection selection)
        {
            var copied = 0;
            ...
            foreach (var job in selection.Jobs)
            {
                ...
                {
                    Debug.LogWarning($"[Development Copy] No weapon/shield sprite mapping exists for job '{job.Name}'.");
                    continue;
                }
            ...
            foreach (var sound in selection.EffectSounds)
            {
                ...
                if (source == null)
                {
                    Debug.LogWarning($"[Development Copy] Optional sound not found: {sound}");
                    continue;
                }
        ...
        private static void CopyFullDevelopmentProfileData(string dataDir)
        {
